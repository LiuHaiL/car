...
if (!function_exists('generateRandomIdNumber')) {
    function generateRandomIdNumber() {
        // 地址码（前6位），这里随机生成一个合法的县（市、旗、区）行政区划代码
        // 注意：这里只是示例，实际中应该使用真实的行政区划代码
        $addressCode = str_pad(rand(100000, 999999), 6, '0', STR_PAD_LEFT);
     
        // 出生日期码（第7-14位），随机生成一个合法的日期
        $startYear = 1950; // 假设从1950年开始
        $endYear = date('Y') - 18; // 减去18岁，因为最小年龄假设为18岁
        $year = str_pad(rand($startYear, $endYear), 4, '0', STR_PAD_LEFT);
        $month = str_pad(rand(1, 12), 2, '0', STR_PAD_LEFT);
        $day = str_pad(rand(1, 28), 2, '0', STR_PAD_LEFT); // 简化处理，不考虑每个月的具体天数
     
        // 顺序码（第15-17位），随机生成一个奇数或偶数（根据性别）
        $gender = rand(0, 1); // 0表示女性（偶数），1表示男性（奇数）
        ...
        // 拼接前17位
        $idNumber = $addressCode . $year . $month . $day . $sequenceCode;
     
        // 校验码（第18位），这里不计算，只生成一个随机数字作为占位符
        // 注意：实际中应该根据前17位数字计算校验码
        $checkCode = str_pad(rand(0, 9), 1, '0', STR_PAD_LEFT);
     
        // 拼接完整的身份证号码
        $fullIdNumber = $idNumber . $checkCode;
...
if (!function_exists('full_url')) {
    ...
    function full_url(string $relativeUrl = '', string|bool $domain = true, string $default = ''): string
    {
        ...
        if (!$cdnUrl) $cdnUrl = request()->upload['cdn'] ?? 'http//' . request()->host();
        if ($domain === true) {
            $domain = $cdnUrl;
        } elseif ($domain === false) {
            $domain = '';
